...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: investors } = await supa.from("investor_profiles").select("investment_focus,preferred_sdgs,preferred_countries,min_investment,max_investment");
    let aligned = 0;
    ...
    const summary = JSON.stringify({
      ...
      sector: org.sector, country: org.country, sdgs: org.sdgs, stage: org.stage,
      funding_required: org.funding_required, readiness_score: org.readiness_score,
      verified: org.is_verified, aligned_funders_in_network: aligned,
    });

    const result = await callLovableAI([
      { role: "system", content: "You are an experienced grants advisor. Estimate the probability (0-100) that an NGO will attract genuine funder interest within 6 months given its profile, readiness, and the alignment available in this platform's funder network. Be realistic." },
